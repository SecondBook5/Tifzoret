...
QC uses DESeq2 variance stabilization and reports library size, detected genes, zero-count and mitochondrial fractions, expression densities, PCA, Pearson correlations, Euclidean distances, and variable-gene heatmaps. PCA group envelopes use the within-group covariance when at least three samples are available. For nearly collinear three-point groups, the visual envelope uses a documented minimum 0.20 minor-to-major axis ratio and expands to contain every observed point. DESeq2 models arbitrary formulas and explicit contrasts; apeglm provides shrunken effect sizes.

Enrichment includes hypergeometric ORA with the tested gene universe, fgseaMultilevel preranked enrichment, ssGSEA/GSVA with limma contrasts, GO BP and KEGG views, and multi-track enrichment curves. Focused curves may be selected from configured gene panels; those results are labeled as configured programs rather than database pathways. Random seeds and provider snapshots are recorded.
