...
| [03_differential/term_test.R](../src/tifzoret/workflow/scripts/03_differential/term_test.R) | One full-vs-reduced likelihood-ratio test within a family | Model-based corroboration of the Wald estimands: does the gene need this term at all? Generalizes the old multi-level omnibus into a family of nested comparisons derived automatically from the design | `family_term_test` (core.smk) | r.yaml |
...
| [estimands.R](../src/tifzoret/workflow/scripts/estimands.R) | Cell weights → contrast vector mapping and term test specs | The single authority on what an estimand means numerically; sourced by family_fit.R, estimand.R, term_test.R, de_confirm.R so DESeq2 and edgeR can never disagree on a contrast's sign |
| [de_render.R](../src/tifzoret/workflow/scripts/03_differential/de_render.R) | Differential expression rendering (volcano, MA, PCA, heatmap) | Shared rendering logic sourced by estimand.R; display-sample selection driven by cell membership rather than contrast type |
