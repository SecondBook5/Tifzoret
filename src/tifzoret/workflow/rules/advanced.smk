...
rule contrast_wgcna:
    input: vst=qc("objects/vst.rds"), samples=SAMPLES, annotation=ANNOTATION, contrasts=CONTRASTS, config=str(CONFIG_PATH), script=str(WORKFLOW_ROOT / "scripts" / "07_networks" / "wgcna.R"), utils=UTILS_R
...
rule contrast_curvature:
    input: edges=analysis("advanced/wgcna", "tables/coexpression_edges.tsv"), nodes=analysis("advanced/wgcna", "tables/coexpression_nodes.tsv"), config=str(CONFIG_PATH), script=str(WORKFLOW_ROOT / "scripts" / "07_networks" / "curvature.py")
    output: CURVATURE_PATTERNS
    log: analysis("advanced/curvature", "logs/curvature.log")
    conda: NETWORK_ENV
    shell: "python {input.script} --project-config {input.config:q} --edges {input.edges:q} --nodes {input.nodes:q} --contrast-id {wildcards.contrast_id:q} --outdir {RESULTS}/contrasts/{wildcards.contrast_id}/analyses/advanced/curvature > {log:q} 2>&1"

rule contrast_mediation:
    input: scores=analysis("pathways", "tables/gsva_scores.tsv"), samples=SAMPLES, contrasts=CONTRASTS, config=str(CONFIG_PATH), script=str(WORKFLOW_ROOT / "scripts" / "08_causal" / "mediation.R"), utils=UTILS_R
...
rule contrast_mediation_power:
    input: mediation=analysis("advanced/mediation", "tables/mediation_inputs.tsv"), script=str(WORKFLOW_ROOT / "scripts" / "08_causal" / "power.py")
...
rule contrast_multilayer:
    input: grn=analysis("regulators", "tables/grn_edges_displayed.tsv"), hubs=analysis("advanced/wgcna", "tables/wgcna_hubs.tsv"), string_up=analysis("networks", "tables/string_up_edges.tsv"), string_down=analysis("networks", "tables/string_down_edges.tsv"), script=str(WORKFLOW_ROOT / "scripts" / "07_networks" / "multilayer.py")
