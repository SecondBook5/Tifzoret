...
if (enabled("go")) {
  ...
  go <- go[!is.na(go$SYMBOL) & !is.na(go$GOALL) & go$ONTOLOGYALL == "BP", , drop = FALSE]
  ...
  sets <- bind_rows(sets, data.frame(term = paste0("GO_BP_", go$GOALL), description = unname(go_terms[go$GOALL]), gene_symbol = go$SYMBOL, provider = "go"))
  provider_versions$go <- list(orgdb = orgdb_package, package = as.character(utils::packageVersion(orgdb_package)))
}
...
receipt <- list(
  schema_version = 1,
  provider = "BulkRNAFrame gene-set resolver",
