...
if (nrow(displayed)) {
  plot <- ggplot(displayed, aes(gene_ratio, term_label, size = count, colour = negative_log10_adjusted_p)) +
    ...
    labs(title = "GO Biological Process over-representation", subtitle = "Top terms per direction; complete GO and KEGG results remain in ontology.tsv", x = "Gene ratio", y = NULL) +
    theme_publication(8.1) + theme(panel.grid.major.y = element_blank(), strip.text = element_text(face = "bold"))
} else {
  plot <- empty_plot("GO Biological Process over-representation", "No GO BP terms passed the configured criteria")
}
